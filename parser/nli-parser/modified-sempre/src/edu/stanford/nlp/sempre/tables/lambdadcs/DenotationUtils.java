...
public final class DenotationUtils {
  ...
  /**
   * Added. --Ofer Givoli
   * Code copied from {@link DateProcessor} and modified.
   */
  public static class TimeProcessor extends TypeProcessor {
    public static TimeProcessor singleton = new TimeProcessor();

    @Override
    public boolean isCompatible(Value v) {
      return v instanceof TimeValue;
    }

    @Override
    public int compareValues(Value v1, Value v2) {
      TimeValue t1 = ((TimeValue) v1), t2 = ((TimeValue) v2);
  ...
  public static TypeProcessor getTypeProcessor(Collection<Value> values1, Collection<Value> values2) {
    // modified this method to support TimeValue. --Ofer Givoli
    boolean canBeNumber = true, canBeDate = true, canBeTime = true;
    for (Value value : values1) {
      if (!(value instanceof NumberValue)) canBeNumber = false;
      if (!(value instanceof DateValue)) canBeDate = false;
      if (!(value instanceof TimeValue)) canBeTime = false;
    }
    ...
    if (canBeNumber) {
      ...
      return TimeProcessor.singleton;
    } else {
