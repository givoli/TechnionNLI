    ...
    while (current instanceof JoinFormula) {
      ...
      if (rid1 != null && rid2 != null) {
        ...
        if (containsStrategy("doubleNext")) {
          // In the following two lines, changed "fb:row.row.next" to TableTypeSystem.ROW_NEXT_VALUE.id. --Ofer Givoli
          if ((rid1.equals(TableTypeSystem.ROW_NEXT_VALUE.id) && rid2.equals(TableTypeSystem.ROW_NEXT_VALUE.id)) ||
              (rid1.equals("!" + TableTypeSystem.ROW_NEXT_VALUE.id) && rid2.equals("!" + TableTypeSystem.ROW_NEXT_VALUE.id))) {
            if (DerivationPruner.opts.pruningVerbosity >= 2)
