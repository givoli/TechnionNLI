...
public abstract class ParserState {
  ...
  public List<Derivation> gatherTokenAndPhraseDerivations() {
    ...
    if (!ExperimentRunner.isExperimentCurrentlyRunning()) // Added this condition (to exclude the following code from NLI runs) --Ofer Givoli
    {
      // All tokens (length 1)
  ...
  protected void ensureExecuted() {
    ...
    for (Derivation deriv : predDerivations) {
      ...
      if (ex.targetValue != null) {
        ...
        CallbacksFromSempre.reportDerivCorrectnessWasSet(deriv,deriv.compatibility == 1); //Added. --Ofer Givoli
      }
  ...
  public static void computeExpectedCounts(List<Derivation> derivations, Map<String, Double> counts) {
    ...
    if (ExperimentRunner.isExperimentCurrentlyRunning()) // Added block. --Ofer Givoli
      ExperimentRunner.getCurrentExperiment().postprocessGradient(counts);
  }
  ...
  /**
   * Added method. --Ofer Givoli
   * Dirty hack due to {@link FloatingParserState} being not public
   * @return -1 in case the derivation isn't in any floating cells.
   */
