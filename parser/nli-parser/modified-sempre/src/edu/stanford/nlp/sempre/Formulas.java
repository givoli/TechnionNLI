...
public abstract class Formulas {
  public static Formula fromLispTree(LispTree tree) {
    ...
    { // Create a function call.    // Added. --Ofer Givoli
      if (NliMethodCallFormula.KEYWORD_IN_GRAMMAR_LISP_TREE.equals(func)) {
        return new NliMethodCallFormula(tree);
      }
    }
  ...
  public static boolean containsFreeVar(Formula formula, VariableFormula var) {
    ...
    if (formula instanceof NliMethodCallFormula) { // Added. --Ofer Givoli.
      NliMethodCallFormula fc = (NliMethodCallFormula) formula;
      return fc.getAllStrictlySubFormulasContained().stream().anyMatch(c->containsFreeVar(c,var));
    }
  ...
  private static void extractSubpartsRecursive(Formula f, Set<String> res) {
    ...
    } else if (f instanceof NliMethodCallFormula) { //Added. --Ofer Givoli
      NliMethodCallFormula functionCall = (NliMethodCallFormula) f;
      functionCall.getAllStrictlySubFormulasContained().forEach(c->extractSubpartsRecursive(c, res));
    }
  }
