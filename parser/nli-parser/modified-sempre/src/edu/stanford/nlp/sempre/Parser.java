...
public abstract class Parser {
  ...
  public ParserState parse(Params params, Example ex, boolean computeExpectedCounts) {
    ...
    CallbacksFromSempre.reportSempreStartingInference(ex); // Added. --Ofer Givoli
    ...
    CallbacksFromSempre.reportInferenceResults(ex, state.parseTime, ex.evaluation.getFig("correct").mean()); // There's a single result which is thus equal to the mean. Added. --Ofer Givoli
    assert ex.evaluation.getFig("correct").count() == 1; // Added. --Ofer Givoli
    ...
    CallbacksFromSempre.reportSempreFinishedInference(); // Added. --Ofer Givoli
  ...
  public void addToEvaluation(ParserState state, Evaluation evaluation) {
    ...
    CallbacksFromSempre.reportDerivationsPredictedByInference(ex, predDerivations, compatibilities, numTop, state); // Added. --Ofer Givoli.
