...
//    System.exit(exitCode);  commented out. --Ofer Givoli
  }
  ...
  public static void runWithObjArray(String[] args, Object[] objects)
  {
    ...
    if (Boolean.getBoolean("debug")){
      // We don't catch the exception. --Ofer Givoli
      ((Runnable)mainObj).run();
    } else {
      // Original logic. --Ofer Givoli
      try {
        ((Runnable) mainObj).run();
      } catch (Throwable t) {
        raiseException(t);
        LogInfo.flush();
        monitorThread.finish(); // added. (Otherwise the program won't exit) --Ofer Givoli
        throw t; // added. We want exceptions to keep propagating (also, without this the exception message is written to stdout) --Ofer Givoli
      }
    }
    finish();
  }
